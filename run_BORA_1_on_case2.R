...
for( t in 1:t.max ) {
  ...
  if( t > n.arms+1 && sd(y)!=0 ) {
    ...
    cat(" - RS...\n")
    x.next = round(runif(n.arms)*(search.space$upper-search.space$lower) + search.space$lower,2)
...
if( save.results ) {
  ...
  saveRDS( results, paste0("results_case2/BORA1_",seed,"_constbgt_",constant.budget,"_arms_",n.arms,".RDS") )
}
