...
#   lines( b1.m+b1.s, col="green3" ); lines( b1.m-b1.s, col="green3" )
...
# #          col=c("green3","green3","red","orange"), cex=1.5)
# # } else {
...
plot( b1.m, type="l", ylim=c(1,Y.MAX), main=paste("15 arms","-",ifelse(display.const.budget,"Constant budget","changing budget")),
      cex.axis=1.5, cex.lab=1.5, xlab="time-step t", ylab=expression(paste("Cumulative Reward (",log[10],")" )) )
if( use.shaded.area ) {
  polygon( c(1:length(b1.m),length(b1.m):1), c(b1.m+b1.s,rev(b1.m-b1.s)),
           col=adjustcolor("green3",alpha.f=0.2), border=NA )
...
if( use.bordered.area) {
  lines( b1.m+b1.s, col="green3" ); lines( b1.m-b1.s, col="green3" )
  lines( b2.m+b2.s, col="red" ); lines( b2.m-b2.s, col="red" )
}

lines( b1.m, col="green3",lwd=4 )
lines( b2.m, col="red",lwd=4 )

legend("bottomright",legend=c("CBO","BORA"),lwd=4,
       col=c("green3","red"), cex=1.5)
