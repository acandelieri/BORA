...
for( t in 1:t.max ) {
  ...
  if( t > length(nu)+1 && sd(y)!=0 ) {
    cat(" - CBO...\n")
    res = cbo( X=as.data.frame(X), y=y, r.budget=bs[t], search.space=search.space,
               maximize=T, beta_=1, covtype="gauss", nugget.estim=T )
    x.next = round(res$res$par,2)
  } else {
    cat(" - RS...\n")
    x.next = round( runif(1) * (search.space$upper[1]-search.space$lower[1]) + search.space$lower[1], 2 )
    x.next = min( x.next, bs[t] )
    x.next = round(c( x.next, bs[t]-x.next ),2)
  }
...
if( save.results ) {
  results = data.frame( x1=X[,1], x2=X[,2], y=y )
  if( !dir.exists("results_case1") )
    dir.create("results_case1")
  saveRDS( results, paste0("results_case1/BORA1_stoch_",stochastic,"_",seed,"_constbgt_",constant.budget,".RDS") )
}
