...
for( f in files ) {
  ...
  if( algo %in% c("BORA2","BORA3","BORA3rescaled","LATTIMORE2014optinit","LATTIMORE2014rndinit") ) {
    N = (ncol(ds)-1)/2
    ds = ds[,-c((N+1):(2*N))]
...
# lat2014opt = df[which(df$algo=="LATTIMORE2014optinit" & df$constant.budget==display.const.budget & df$iter<=lim.T), ]
...
if( !is.constant.budget ) {
  legend("bottomright",legend=c("BORA1","BORA2","BORA3","BORA3_rescaled","SBF"),lwd=3,
         col=c("blue","green3","red","orange","purple"), cex=1.5)
} else {
...
#          col=adjustcolor("red",alpha.f=0.2), border=NA )
# lines( t0:tf, b1.m[t0:tf], col="blue",lwd=4 )
...
# # plot(density(apply(b1M,2,sum)),col="blue", ylim=c(0,0.3), lwd=3 )
