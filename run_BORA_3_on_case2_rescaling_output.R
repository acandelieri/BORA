...
for( t in 1:t.max ) {
  ...
  if( t > n.arms+1 && sd(y)!=0 ) {
    cat(" - BORA3...\n")
    cat("* 'y' values rescaled according to budget!\n")
    res = bora3( A=A, y=y/bs[1:(t-1)], maximize=T, beta_=1, n.proc=n.proc )
...
if( save.results ) {
  ...
  saveRDS( results, paste0("results_case2/BORA3rescaled_",seed,"_constbgt_",constant.budget,"_arms_",n.arms,".RDS") )
}